// CLAIR Dissertation Layout Module
// Exports all layout functions and styles
...
#let chapter_title(chapter_num: auto, title: "", epigraph: none) = {
  let epigraph_block = if epigraph != none {
    [
      #v(2cm)
      #align(center)[
        #set text(
  ...
  let roman_numerals = ("I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X",
                        "XI", "XII", "XIII", "XIV", "XV")
  let chapter_roman = if chapter_num != auto and chapter_num <= roman_numerals.len() {
    roman_numerals.at(chapter_num - 1)
  } else if chapter_num != auto {
    str(chapter_num)
  } else {
    ""
  }
...
#let title_page() = [
  ...
  #block(width: 12cm)[Comprehensible LLM AI Intermediate Representation]

...
theorem rebut_zero_against (c : Confidence) (hc : (c : ℝ) ≠ 0) :
  ...
  · exfalso; exact hc h
  · apply Subtype.ext
    simp only [Subtype.coe_mk, unitInterval.coe_one]
    field_simp

/-- No supporting evidence means full defeat -/
theorem rebut_zero_for (c : Confidence) (hc : (c : ℝ) ≠ 0) :
    rebut 0 c = 0 := by
  simp only [rebut, unitInterval.coe_zero, zero_add]
  split_ifs with h
  · exfalso; exact hc h
  · apply Subtype.ext
    simp only [Subtype.coe_mk, unitInterval.coe_zero]
    field_simp
...
theorem rebut_eq (c : Confidence) (hc : (c : ℝ) ≠ 0) :
    ((rebut c c) : ℝ) = 1/2 := by
  simp only [rebut]
  split_ifs with h
...
theorem rebut_add_rebut_swap (a b : Confidence) (h : (a : ℝ) + (b : ℝ) ≠ 0) :
  ...
  simp only [rebut]
  have h' : (b : ℝ) + (a : ℝ) ≠ 0 := by linarith [add_comm (a : ℝ) (b : ℝ)]
...
theorem rebut_mono_for (a a' b : Confidence)
  ...
  simp only [rebut]
  split_ifs with h1 h2
  · exfalso; exact hpos h1
  · exfalso; exact hpos h1
  · simp only [Subtype.coe_mk]
    have pos : 0 < (a' : ℝ) + (b : ℝ) := by
      have nonneg := add_nonneg a'.nonneg b.nonneg
      cases' (nonneg.lt_or_eq) with hlt heq
      · exact hlt
    ...
    nlinarith [a.nonneg, a'.nonneg, b.nonneg]
  · simp only [Subtype.coe_mk]
    have pos : 0 < (a' : ℝ) + (b : ℝ) := by
      have nonneg := add_nonneg a'.nonneg b.nonneg
      cases' (nonneg.lt_or_eq) with hlt heq
...
theorem rebut_anti_against (a b b' : Confidence)
  ...
  · simp only [Subtype.coe_mk]
    ...
    have pos' : 0 < (a : ℝ) + (b' : ℝ) := by
      ...
      · exfalso; exact hpos' heq.symm
    rw [div_le_div_iff pos' pos]
    nlinarith [a.nonneg, b.nonneg, b'.nonneg]
